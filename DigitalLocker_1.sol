...
contract DigitalLocker
{
    ...
    function RejectApplication(string memory rejectionReason) public
    {
        ...
        LockerStatus = "Rejected";  // Corrección de "Rejeced" a "Rejected"
        State = StateType.DocumentReview;

        assert(keccak256(bytes(LockerStatus)) == keccak256(bytes("Rejected")));
        assert(keccak256(bytes(RejectionReason)) == keccak256(bytes(rejectionReason)));
    }
